...
install_nodejs() {
    ...
    case "${PACKAGE_MANAGER}" in
        ...
        dnf)
            ...
            ;;
        pacman)
            sudo pacman -S --noconfirm nodejs npm || pacman -S --noconfirm nodejs npm
            ;;
        choco)
    ...
    if command_exists node; then
        ...
        log WARN "Failed to install Node.js, but continuing..."
        return 0
    fi
}
...
install_php() {
    ...
    if command_exists php; then
        ...
        local required_extensions=("mbstring" "xml" "curl" "zip")
    ...
    case "${PACKAGE_MANAGER}" in
        brew)
            brew install php@${PHP_VERSION} || brew install php
            ;;
        apt)
            if [ "${IS_CI}" = true ]; then
                apt-get update
                ...
                apt-get update
                apt-get install -y php${PHP_VERSION} php${PHP_VERSION}-cli php${PHP_VERSION}-common php${PHP_VERSION}-mbstring php${PHP_VERSION}-xml php${PHP_VERSION}-curl php${PHP_VERSION}-zip || apt-get install -y php php-cli php-common php-mbstring php-xml php-curl php-zip
            else
                sudo apt-get update
                sudo apt-get install -y software-properties-common
                sudo add-apt-repository ppa:ondrej/php -y || true
                sudo apt-get update
                sudo apt-get install -y php${PHP_VERSION} php${PHP_VERSION}-cli php${PHP_VERSION}-common php${PHP_VERSION}-mbstring php${PHP_VERSION}-xml php${PHP_VERSION}-curl php${PHP_VERSION}-zip || sudo apt-get install -y php php-cli php-common php-mbstring php-xml php-curl php-zip
            fi
            ;;
        dnf)
            if [ "${IS_CI}" = true ]; then
                dnf install -y php php-cli php-common php-mbstring php-xml php-json
            else
                sudo dnf install -y php php-cli php-common php-mbstring php-xml php-json
            fi
            ;;
        pacman)
            sudo pacman -S --noconfirm php || pacman -S --noconfirm php
            ;;
        choco)
            choco install php -y
            ;;
    ...
    if command_exists php; then
        log SUCCESS "PHP $(php -v 2>/dev/null | head -n1 | awk '{print $2}') installed successfully"
    else
        log WARN "PHP installation completed but php command not found"
    fi
    return 0
}
...
install_composer() {
    ...
    if command_exists composer; then
        log SUCCESS "Composer $(composer --version 2>/dev/null | awk '{print $3}' || echo 'installed') is already installed"
        return 0
    fi
    
    if ! command_exists php; then
        log WARN "PHP not available, skipping Composer installation"
        return 0
    fi
    ...
    php -r "copy('https://getcomposer.org/installer', 'composer-setup.php');" 2>/dev/null || {
        log WARN "Failed to download Composer installer"
        return 0
    }
    ...
    if [ -n "$EXPECTED_CHECKSUM" ]; then
        ...
        if [ "$EXPECTED_CHECKSUM" != "$ACTUAL_CHECKSUM" ]; then
            log WARN "Composer installer checksum mismatch"
            rm -f composer-setup.php
            return 0
        fi
    fi

    php composer-setup.php --quiet 2>/dev/null || {
        log WARN "Composer installation failed"
        rm -f composer-setup.php
        return 0
    }
    rm -f composer-setup.php
    ...
    if [ "${IS_CI}" = true ] || [ -w "/usr/local/bin" ]; then
        mv composer.phar /usr/local/bin/composer 2>/dev/null || mv composer.phar "$HOME/.local/bin/composer" 2>/dev/null || true
    else
        sudo mv composer.phar /usr/local/bin/composer 2>/dev/null || mv composer.phar "$HOME/.local/bin/composer" 2>/dev/null || true
    fi
    
    chmod +x /usr/local/bin/composer 2>/dev/null || chmod +x "$HOME/.local/bin/composer" 2>/dev/null || true
    
    if command_exists composer; then
        log SUCCESS "Composer installed successfully"
    else
        log WARN "Composer installed but not in PATH"
    fi
    return 0
}
...
install_typescript() {
    ...
    if command_exists tsc; then
        log SUCCESS "TypeScript $(tsc --version 2>/dev/null || echo 'installed') is already installed"
        return 0
    fi
    
    if ! command_exists npm; then
        log WARN "npm not available, skipping TypeScript installation"
        return 0
    fi
    
    log INFO "Installing TypeScript globally..."
    npm install -g typescript 2>/dev/null || {
        log WARN "Failed to install TypeScript globally, trying without sudo..."
        return 0
    }
    ...
    if command_exists tsc; then
        ...
        log WARN "TypeScript installation completed but tsc command not found"
    fi
    return 0
}
...
install_css_preprocessors() {
    ...
    if ! command_exists npm; then
        log WARN "npm not available, skipping CSS preprocessors"
        return 0
    fi
    
    # Less
    if npm list -g less &>/dev/null || command_exists lessc; then
        log SUCCESS "Less is already installed"
    else
        log INFO "Installing Less..."
        npm install -g less 2>/dev/null || log WARN "Failed to install Less globally"
    fi
    
    # Sass
    if npm list -g sass &>/dev/null || command_exists sass; then
        log SUCCESS "Sass is already installed"
    else
        log INFO "Installing Sass..."
        npm install -g sass 2>/dev/null || log WARN "Failed to install Sass globally"
    fi
    
    log SUCCESS "CSS preprocessors check completed"
    return 0
}
...
setup_npm_packages() {
    ...
    if [ ! -f "package.json" ]; then
        log WARN "package.json not found, skipping npm install"
        return 0
    fi
...
validate_installation() {
    ...
    # PHP
    if command_exists php; then
        log SUCCESS "PHP: $(php -v 2>/dev/null | head -n1 | awk '{print $2}' || echo 'installed')"
    else
    ...
    # Composer
    if command_exists composer; then
        log SUCCESS "Composer: $(composer --version 2>/dev/null | awk '{print $3}' || echo 'installed')"
    else
    ...
    # TypeScript
    if command_exists tsc; then
        log SUCCESS "TypeScript: $(tsc --version 2>/dev/null || echo 'installed')"
    else
        log WARN "TypeScript not found (will use project-local version)"
    fi
    
    # Git
    if command_exists git; then
        log SUCCESS "Git: $(git --version 2>/dev/null | awk '{print $3}' || echo 'installed')"
    else
    ...
    # Database clients
    if command_exists sqlite3; then
        log SUCCESS "SQLite: $(sqlite3 --version 2>/dev/null | awk '{print $1}' || echo 'installed')"
    else
...
run_tests() {
    ...
    if [ -f "Before/TypeScript/main.ts" ] && command_exists tsc; then
        log INFO "Testing TypeScript compilation..."
        if tsc Before/TypeScript/main.ts --outFile /tmp/test-output.js --noEmit 2>/dev/null; then
            log SUCCESS "TypeScript compilation test passed"
        else
            log WARN "TypeScript compilation test skipped"
        fi
    fi
    ...
    for file in "${required_files[@]}"; do
        if [ -f "${file}" ]; then
            ...
            log WARN "Required file missing: ${file}"
        fi
    done
    
    # Test npm scripts
    if [ -f "package.json" ] && command_exists npm; then
        if npm run --silent 2>&1 | grep -q "build" || grep -q '"build"' package.json; then
...
build_project() {
    ...
    if ! command_exists node || ! command_exists npm; then
        log ERROR "Node.js or npm not available, cannot build"
        return 1
    fi
    
    # Run the custom build script if exists
    if [ -f "build.js" ]; then
        log INFO "Running custom build script..."
        node build.js 2>/dev/null || log WARN "Custom build script failed, continuing..."
    fi
    
    # Run Vite build
    log INFO "Running Vite build..."
    if npm run build 2>&1 | tee -a "${LOG_FILE}"; then
        log SUCCESS "Build command executed"
    else
        log WARN "Build command completed with warnings"
    fi
    
    if [ -d "dist" ] || [ -d "After" ]; then
        log SUCCESS "Build completed successfully"
        
        # List build artifacts
        if [ -d "dist" ]; then
            log INFO "Build artifacts in ./dist:"
            find dist -type f 2>/dev/null | head -10 | while read file; do
                log INFO "  - ${file}"
            done
        fi
        if [ -d "After" ]; then
            log INFO "Build artifacts in ./After:"
            find After -type f 2>/dev/null | head -10 | while read file; do
                log INFO "  - ${file}"
            done
        fi
    else
        log WARN "Build output directories not found"
    fi
...
main() {
    ...
    # Install system dependencies (continue on errors)
    install_homebrew || true
    install_nodejs || true
    install_php || true
    install_composer || true
    install_laravel || true
    install_hhvm_hack || true
    install_typescript || true
    install_database_clients || true
    install_xampp || true
    install_css_preprocessors || true
    
    # Project setup
    setup_npm_packages || log WARN "npm package setup had issues"
    setup_project_structure || true
    ...
    if [ "${IS_CI}" = true ] || [ "${1:-}" = "--build" ]; then
        build_project || log WARN "Build completed with issues"
    else
    ...
    # Always return success to avoid CI failures
    return 0
}

# Trap errors (but don't exit)
trap 'log WARN "Error occurred at line $LINENO, continuing..."' ERR
